...
class YamlCalConverter():
    ...
    def generate_series_datetimes(start_date, periods, start_time, end_time,
                                  days, repeat, tz, start_offset, end_offset):
        d = start_date
        results = []
        ...
        for i in xrange(repeat):
            ...
            if start_offset:
              start += timedelta(minutes=start_offset)
            if end_offset:
              end += timedelta(minutes=end_offset)
    ...
    def _convert(self):
        ...
        for event in self.events:
            ...
            dates = YamlCalConverter.generate_series_datetimes(
                ...
                event['end_time'], days, repeat, tz, self.options['start_offset'], self.options['end_offset'])
